...
func (s *server) acceptConnections() {
	for {
		...
		if err != nil {
			fs.Errorf(what, "Failed to handshake (%s)", err)
			continue
		}

		fs.Infof(what, "New SSH connection from %s (%s)", sshConn.RemoteAddr(), sshConn.ClientVersion())
...
func (s *server) serve() (err error) {
	...
	s.config = &ssh.ServerConfig{
		...
		AuthLogCallback: func(conn ssh.ConnMetadata, method string, err error) {
			...
			fs.Infof(describeConn(conn), "ssh auth %q from %q: %s", method, conn.ClientVersion(), status)
		},
		NoClientAuth: s.opt.NoAuth,
	}

	// Set PasswordCallback if a user and pass is configured
	if s.opt.User != "" && s.opt.Pass != "" {
		s.config.PasswordCallback = func(c ssh.ConnMetadata, pass []byte) (*ssh.Permissions, error) {
			fs.Debugf(describeConn(c), "Login: %s\n", c.User())
